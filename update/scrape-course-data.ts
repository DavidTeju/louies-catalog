...
export async function getDepartmentUrlsAndNames(): Promise<DepartmentInfo[]> {
    ...
    return $('.letternav-head').map((_, element) => {
    ...
    function listItemToUrl(_: any, listItem: cheerio.Element) {
        ...
        return {departmentUrl: domain + link, departmentName: $(listItem).text().trim()};
    }
}


export async function getDepeartmentCourses({departmentUrl, departmentName}: DepartmentInfo): Promise<{
            ...
            .forEach((element) => {
                ...
                switch (option) {
                    case 'Term Typically Offered:':
                        termOffered = $(element).parent().text()
                            .replace('Term Typically Offered:', '')
                            .split(',').map((str) => str.trim());
                        break;
                    case 'Prerequisite(s):':
                        prerequisites = $(element).parent().text()
                            .replace('Prerequisite(s):', '').trim();
                        break;
                    case 'Description:':
                        description = $(element).parent().text()
